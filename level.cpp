    ...
    for (int y = 0; y < GetMapSize(); y++) {
      if (tileMap[x][y].IsOccupied() &&
        ...
        if (tileMap[x][y].GetGridLocationX() <
          ...
          // (static_cast<unsigned>((tileMap[x][y].GetEnemy()->GetPosX()))) <<
          // std::endl; std::cout << "Next x pos: " <<
          // (static_cast<unsigned>((tileMap[x][y].GetNext()->GetGridLocationX())))
          // << std::endl;
  ...
  for (int x = 0; x < GetMapSize(); x++) {
    for (int y = 0; y < GetMapSize(); y++) {
      if (((tileMap[x][y].type_ == 1) || (tileMap[x][y].type_ == 2)) &&
        ...
        tileMap[x][y].GetEnemy()->ChangePos(
            ...
            tileMap[x][y].GetEnemy()->GetPosY());
                 ...
                 (tileMap[x][y].GetEnemy()->GetPosX() !=
                  ...
                  tileMap[x][y].GetEnemy()->GetPosY() !=
                      tileMap[x][y].GetNext()->GetGridLocationY() * 1.f)) {
        // std::cout << "EnemyPos: " << tileMap[x][y].GetEnemy()->GetPosX() << "
        // " << tileMap[x][y].GetEnemy()->GetPosY() << std::endl; std::cout <<
        // "Next tile Pos: " << tileMap[x][y].GetNext()->GetGridLocationX() << "
        // " << tileMap[x][y].GetNext()->GetGridLocationY() << std::endl;
        // std::cout << (tileMap[x][y].GetEnemy()->GetPosX() !=
        // tileMap[x][y].GetNext()->GetGridLocationX()) * 1.f << std::endl;
        tileMap[x][y].GetEnemy()->ChangePos(
            tileMap[x][y].GetEnemy()->GetPosX(),
        ...
        // std::endl; std::cout << "Next x pos: " <<
        // (static_cast<double>((tileMap[x][y].GetNext()->GetGridLocationX())))
        // << std::endl; std::cout << "Left pixel movement" << std::endl;
        tileMap[x][y].GetEnemy()->ChangePos(
        ...
        tileMap[x][y].GetEnemy()->ChangePos(
            tileMap[x][y].GetEnemy()->GetPosX(),
            tileMap[x][y].GetEnemy()->GetPosY() - (1.f / 100));
      }
      if (((tileMap[x][y].type_ == 1) || (tileMap[x][y].type_ == 2))) {
        // std::cout << "Type check: " << (tileMap[x][y].type_ == 1) <<
        // std::endl;
        if (tileMap[x][y].IsOccupied()) {
          // std::cout << "Tile occupied test: " << (tileMap[x][y].IsOccupied())
          // << std::endl; std::cout << "Enemy mover flag test: " <<
          // (tileMap[x][y].enemyMovedHere) << std::endl; std::cout << "Next not
          // occupied test: " << (!tileMap[x][y].GetNext()->IsOccupied()) <<
          // std::endl; std::cout << "Same X pos test: " <<
          // (static_cast<double>(tileMap[x][y].GetEnemy()->GetPosX()) ==
          // static_cast<double>(tileMap[x][y].GetNext()->GetGridLocationX()
          // * 1.f)) << std::endl; std::cout << "Same Y pos test: " <<
          // (static_cast<double>(tileMap[x][y].GetEnemy()->GetPosY() * 1.f) ==
          // static_cast<double>(tileMap[x][y].GetNext()->GetGridLocationY()
          // * 1.f)) << std::endl;
        }
      }
      ...
      if (((tileMap[x][y].type_ == 1) || (tileMap[x][y].type_ == 2)) &&
        ...
        // std::endl; std::cout << "Next x pos: " <<
        // (static_cast<double>((tileMap[x][y].GetNext()->GetGridLocationX())))
        // << std::endl; std::cout << "move one enemy" << std::endl; std::cout
        // << tileMap[x][y].GetEnemy() << std::endl;
        ...
        // std::cout << "Succesfully turned enemyMovedFlag to false" <<
        // std::endl;
        //  tileMap[x][y].GetEnemy()->ChangePos(tileMap[x][y].GetNext()->GetGridLocationX(),
        //  tileMap[x][y].GetNext()->GetGridLocationY()); std::cout <<
        //  "Successfully changed Pos values in enemy instance" << std::endl;
        tileMap[x][y].enemyMoved();
        // std::cout << "Successfully removed enemy pointer from current tile"
        // << std::endl;
      }
    }
  }
